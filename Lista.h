...
class Lista
{
    ...
    public:
        ...
        //Cambia los datos de una posicion determinada
        // PRE: - lista creada y no vacía
        // - 0 < pos <= tam
        // POST: cambia el dato de la posicion "pos" por d 
        void set_dato(unsigned pos, Dato d);
        // ¿Lista vacia?
        // PRE: Lista creada
...
template < typename Dato >
void Lista<Dato>::set_dato(unsigned pos, Dato d){
	Dato* dat;
	dat=this->get_dato(pos);
	*dat=d;
}
...
void Lista<Dato>::delet_list(){
	unsigned pos=1;
	while(!aristas.lista_vacia()){
		aristas.get_dato(pos);
	}
}
